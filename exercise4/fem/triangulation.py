...
elementsDefault = [
    61, 62, 63, 64, 65, 66, 67, 68, 79, 80, 81, 82, 83, 84, 85, 97, 98, 99,
...
class Triangulation:
    ...
    def createTriangulation(self, elements_to_be_modified=elementsDefault):
        if (self.mode in [0, 1, 2, 3, 4]):
            ...
            coords = np.ones(
                (2, (self.numberElementsX + 1) * (self.numberElementsY + 1)))
            ...
            for j in np.arange(self.numberElementsY):
                for i in np.arange(self.numberElementsX):
                    nodesLower = np.array([
                        j * (self.numberElementsX + 1) + i + 1,
                        j * (self.numberElementsX + 1) + i + 2,
                        (j + 1) * (self.numberElementsX + 1) + i + 1
                    ])
                    coordLower = np.array([[
                        coords[0, nodesLower[0] - 1], coords[1,
                                                             nodesLower[0] - 1]
                    ],
                                           [
                                               coords[0, nodesLower[1] - 1],
                                               coords[1, nodesLower[1] - 1]
                                           ],
                                           [
                                               coords[0, nodesLower[2] - 1],
                                               coords[1, nodesLower[2] - 1]
                                           ]])
                    idLower = j * self.numberElementsX * 2 + i * 2 + 1
                    K = self.k
                    if (self.mode == 4 and idLower in elements_to_be_modified):
                        K = K * self.c
                    eLower = Element(coordLower, nodesLower, idLower, K,
                                     self.h)

                    nodesUpper = np.array([
                        (j + 1) * (self.numberElementsX + 1) + i + 1,
                        j * (self.numberElementsX + 1) + i + 2,
                        (j + 1) * (self.numberElementsX + 1) + i + 2
                    ])
                    coordUpper = np.array([[
                        coords[0, nodesUpper[0] - 1], coords[1,
                                                             nodesUpper[0] - 1]
                    ],
                                           [
                                               coords[0, nodesUpper[1] - 1],
                                               coords[1, nodesUpper[1] - 1]
                                           ],
                                           [
                                               coords[0, nodesUpper[2] - 1],
                                               coords[1, nodesUpper[2] - 1]
                                           ]])
                    idUpper = j * self.numberElementsX * 2 + i * 2 + 2
                    K = self.k
                    if (self.mode == 4 and idUpper in elements_to_be_modified):
                        K = K * self.c
                    eUpper = Element(coordUpper, nodesUpper, idUpper, K,
                                     self.h)

                    elementlist.append(eLower)
                    elementlist.append(eUpper)

            M = Mesh(elementlist, self.numberElementsX, self.numberElementsY,
                     self.L, coords)
